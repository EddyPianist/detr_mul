...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, refpoint_embed, pos_embed, tgt, attn_mask=None, feat_maps = None):
        ...
        #for id in range(2):
        #    if id < 1:
        #        feat = self.pooling(feat_maps[(1 - id)])
        #    else:
        #        feat = self.pooling(self.pooling(feat_maps[(1 - id)]))
        #    #we should notice the feature map contains in a upside down manner.
        #    feat = feat.flatten(2).permute(2, 0, 1)
        #    feat = self.feat_proj_list[1 - id](feat)
        #    mul_feat.append(feat)
        #memory = memory + self.mul_feat[0] + self.mul_feat[1]
        ...
        hs, references = self.decoder(tgt, memory, tgt_mask=attn_mask, memory_key_padding_mask=mask,
                          pos=pos_embed, refpoints_unsigmoid=refpoint_embed, feat_maps = feat_maps)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id < 2:
                mul_feat.append(self.pooling(self.pooling(feat_maps[(layer_id//2)])))
                mul_feat = mul_feat[0].flatten(2).permute(2, 0, 1)
                mul_feat = self.feat_input_proj_list[(layer_id//2)](mul_feat)
                #mul_feat = self.fus_attn(memory, mul_feat, mul_feat)[0]
                memory = mul_feat + memory
            elif layer_id < 4:
                mul_feat.append(self.pooling(feat_maps[(layer_id//2)]))
                ...
                mul_feat = mul_feat[0].flatten(2).permute(2, 0, 1)
                mul_feat = self.feat_input_proj_list[(layer_id//2)](mul_feat)
                memory =  mul_feat + memory #we should notice the feature map contains in a upside down manner.
