...
from models import build_DABDETR, build_dab_deformable_detr, build_dab_deformable_detr_deformable_encoder_only, build_DABDETR_MUL
...
def get_args_parser():
    ...
    parser.add_argument('--label_noise_scale', default=0.3, type=float,
    ...
    # about lr
    parser.add_argument('--lr', default=1e-4, type=float,  #orignal: 1e-4
                        help='learning rate')
    ...
    parser.add_argument('--num_feature_levels', default=3, type=int,   #default = 4
...
def build_model_main(args):
    ...
    elif args.modelname.lower() == 'detr_mul':
        model, criterion, postprocessors = build_DABDETR_MUL(args)
    else:
...
def monitor_gradients(model):
    for name, param in model.named_parameters():
        if param.grad is not None:
            avg_grad = param.grad.data.abs().mean()
            print(f"The average absolute gradient of {name} is {avg_grad:.4f}")
