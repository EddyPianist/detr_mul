...
class DABDETR_mul(nn.Module):
    ...
    def forward(self, samples: NestedTensor, dn_args=None):
        ...
        hs, reference = self.transformer(self.input_proj(src), masks[-1], input_query_bbox, pos[-1], tgt=input_query_label,
                                         attn_mask=attn_mask, feat_maps = srcs, masks = masks)    #be careful here, we cannot use src[-1] in srcs later
